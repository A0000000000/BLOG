...
public class BaseActionResult<T> implements Serializable {
    ...
    public static <T> BaseActionResult<T> from(T source, @NonNull Integer code, @NonNull ResultCodeTools tools) {
        BaseActionResult<T> res = new BaseActionResult<>();
        CodeContrast codeContrast = null;
        try {
            codeContrast = tools.getCodeContrast(code);
        } catch (Exception ignore) { }
        if (codeContrast == null) {
            if (CodeContrast.DEFAULT_SUCCESS_BEAN.getCode().equals(code)) {
